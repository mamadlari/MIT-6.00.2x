...
def greedy_cow_transport(cows, limit=10):
    ...
    trips
    """
    # sorted list of touples(cow,weight)
    cowsCopy = sorted(cows.items(), key=lambda x: x[1], reverse=True)
    # initial sorted Cow dicttionary
    sortedCow = dict(cowsCopy)
    sofarWeghit = 0
    takenCow, trip = [], []
    # iterate until take all cows
    while len(sortedCow) != 0:
        for cow, weight in sortedCow.items():
